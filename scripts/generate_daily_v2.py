...
def build_page(meta: dict, enriched_list: list[dict]) -> str:
    ...
    for idx, ((name, slug), items) in enumerate(ordered_groups):
        ...
        for p in items:
            ...
            cards.append(f'''<details class="paper-card deep-card" id="{html.escape(p['id'])}" open><summary><div><div class="badges">{''.join(badges)}</div><div class="paper-title deep-title-en">{html.escape(str(p.get('title_en') or ''))}</div><div class="deep-title-zh">{html.escape(str(p.get('title_zh') or ''))}</div><div class="deep-inst">{html.escape(str(p.get('institution') or '未明确披露'))}</div></div><div class="paper-side"><span class="meta">点击收起</span></div></summary><div class="paper-body deep-body"><div class="info-block brief deep-summary"><div class="info-title">摘要总结</div>{html.escape(str(p.get('summary_cn') or ''))}</div><div class="info-block innovation deep-innovation"><div class="info-title">核心创新点</div><ul class="compact">{innov}</ul></div><div class="info-block scenario deep-scenario"><div class="info-title">解决的问题 / 适用场景</div>{html.escape(str(p.get('scenario_cn') or ''))}</div><div class="info-block links-block"><div class="info-title">论文链接</div><div class="links"><a href="{html.escape(p['abs_url'])}">arXiv</a><a href="{html.escape(p['pdf_url'])}">PDF</a>{code_html}</div></div></div></details>''')
