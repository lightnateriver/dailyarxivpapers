...
SRC = ROOT / 'docs/data/2026-07-13.json'
OUT = ROOT / 'docs/data/2026-07-13.full_enriched.json'
HTML_OUT = ROOT / 'docs/daily/2026-07-13.html'
CSS = ROOT / 'docs/assets/style.css'
CACHE_DIR = ROOT / 'cache' / 'paper_html_2026_07_13'
RESUME_FILE = ROOT / 'cache' / 'paper_html_2026_07_13' / 'resume_enriched.json'
...
def build_page(meta: dict, enriched_list: list[dict]) -> str:
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>2026-07-13 AI Paper Digest</title><link rel="stylesheet" href="../assets/style.css"></head><body><main class="container"><nav class="nav"><a href="../index.html">← 首页</a><span class="meta">2026-07-13</span></nav><section class="hero"><div><div class="eyebrow">Daily Research Board</div><h1>2026-07-13 AI 论文看板</h1><p class="subtitle">本页已按你的要求重建：去掉作者名，加入英文题目+中文翻译，按正文重写摘要总结/创新点/解决场景，并过滤垂直场景和端侧推理论文；每个模块最多保留 5 篇。</p></div><div class="panel"><div class="eyebrow">Selection Rule</div><p class="meta">排序优先级：Ascend / NPU 第一优先，其次已确认开源，再按相关度排序。过滤：营养、农业、无人机、医疗等垂直场景，以及端侧/移动/Edge 推理部署论文。</p></div></section><section class="panel"><div class="summary-bar"><div class="stat"><div class="num">{meta['fetched_count']}</div><div class="label">抓取论文</div></div><div class="stat"><div class="num">{len(enriched_list)}</div><div class="label">整改后保留</div></div><div class="stat"><div class="num">{sum(1 for p in enriched_list if p.get('is_ascend'))}</div><div class="label">Ascend/NPU 优先</div></div><div class="stat"><div class="num">{sum(1 for p in enriched_list if p.get('code_url'))}</div><div class="label">确认开源</div></div></div></section><section class="panel"><div class="eyebrow">Topic Navigation</div><div class="category-nav">{nav}</div></section><section class="board">{''.join(blocks)}</section><footer class="footer">Generated by Daily Arxiv Papers.</footer></main></body></html>'''
